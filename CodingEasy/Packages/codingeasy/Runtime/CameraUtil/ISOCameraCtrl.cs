...
public class ISOCameraCtrl : MonoBehaviour
{
    ...
    void GetOffset()
    {
        ...
        CameraOffset = _root.position - FollowTarget.position;
    }

    [BoxGroup("Offset"), Button("Set"), HorizontalGroup("Offset/1")]
    void AlignCamera()
    {
        var targetPos = FollowTarget.position + CameraOffset;
        _root.position = targetPos;
    }

    public Transform FollowTarget;

    public float MobieModify = 1;
    ...
    {        
        ...
        if (_camera.orthographicSize < ViewSize.x){
            _camera.orthographicSize = ViewSize.x;
            zoomVelocity = 0f;
            return;
        }

        if (_camera.orthographicSize > ViewSize.y){
            _camera.orthographicSize = ViewSize.y;
            zoomVelocity = 0f;
            return;
        }

        if (_camera.orthographicSize + zoomVelocity >= ViewSize.x &&
            _camera.orthographicSize + zoomVelocity <= ViewSize.y ){
            _camera.orthographicSize += zoomVelocity;
        }
    ...
    void TranslateCamera()
    {
        ...
        {
            var touch = Input.GetTouch(0);
            deltaX = touch.deltaPosition.x * MobieModify;
            deltaY = touch.deltaPosition.y * MobieModify;
        }
        else {
    ...
    [Button]
    void ToggleFollowTarget() {
        isFollowTarget = !isFollowTarget;
    }
    [Button]
    void ToggleFocusTarget() {
        isFocusTarget = !isFocusTarget;
        if (isFocusTarget)
        {
            TargetSize = ViewSize.x;
        }
        else {
            TargetSize = ViewSize.y;
        }
...
#region 动画
    ...
    IEnumerator ChangeViewSize()
    {
        ...
        float beginSize = _camera.orthographicSize;
        float time = 0;
        while (time < animationTime)
        {
            time += Time.deltaTime * scaleSpeed;
            var rate = Curve.Evaluate(time / animationTime);
            _camera.orthographicSize = Mathf.Lerp(beginSize, TargetSize, rate);
